function Invoke-LicensingReview {
    ...
    try {
        ...
        Get-MgSubscribedSku -All | ForEach-Object {
            ...
            $skuDisplayNameMap[$_.SkuPartNumber] = $_.AppliesTo + " - " + $_.SkuPartNumber #Example of what I mean: "User - O365_BUSINESS_PREMIUM"
            #Attempt to use a more user-friendly name if available directly from Get-MgSubscribedSku otherwise the report is quite hard to read :D
        ...
        #List of SKUs that include a mailbox (these should be top-level SKUs, don't put friendly names in here if you update)
        $skusWithMailbox = @(
    ...
    foreach ($user in $users) {
        ...
        foreach ($license in $user.AssignedLicenses) {
            ...
            #If the SKU is a top-level license, add it to the list of main licenses and check if it provides a mailbox.
